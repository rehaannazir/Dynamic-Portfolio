...
export const Home = memo(function Home({ setPage }) {
  ...
  const projects = [
    { n: "01", cat: "FastAPI · Gemini", title: "AI Content Automation API", desc: "Production backend that generates, categorizes and schedules content via LLMs behind a clean REST interface.", role: "Full Stack · AI · 2026", stack: ["FastAPI", "Gemini", "SQLAlchemy"], accent: "#a78bfa", viz: "Engineering", metrics: [["p95", "120ms"], ["uptime", "99.9%"], ["routes", "12"]] },
    ...
    { n: "03", cat: "Python · Gemini", title: "Finance Expense Categorizer", desc: "Turns raw CSV bank exports into categorized, analysis-ready Excel sheets automatically.", role: "AI Tooling · 2025", stack: ["Python", "Gemini", "Pandas"], accent: "#a78bfa", viz: "Strategy", metrics: [["rows/run", "1.2k"], ["accuracy", "98%"], ["pipeline", "CSV→XLSX"]] },
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 pt-20 pb-24 grid lg:grid-cols-2 gap-12 items-center">
          <div className="lg:self-start">
            ...
            <div className="fade-up flex flex-wrap gap-3 mt-8" style={{ animationDelay: ".3s" }}>
              <button ref={workRef} onClick={() => setPage("services")} className="btn-glow magnetic inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium text-white" style={{ background: "linear-gradient(135deg,#7c3aed,#8b5cf6)" }}>View selected work <ArrowRight className="w-4 h-4" /></button>
      ...
      <section className="max-w-6xl mx-auto px-5 pb-8">
        <div className="grid sm:grid-cols-3 gap-4">
          <Reveal variant="blur"><div className="float-soft h-full"><div className="glass glass-hover rounded-2xl p-5 h-full">
            <div className="flex items-center justify-between"><span className="mono text-xs text-slate-400">automation activity · 7d</span><GitBranch className="w-4 h-4 text-violet-400" /></div>
          ...
          <Reveal variant="blur" delay={0.1}><div className="float-soft-2 h-full" style={{ animationDelay: "1.2s" }}><div className="glass glass-hover rounded-2xl p-5 h-full">
            <div className="flex items-center justify-between"><span className="mono text-xs text-slate-400">inference · live</span><Activity className="w-4 h-4 text-violet-400" /></div>
          ...
          <Reveal variant="blur" delay={0.2}><div className="float-soft h-full" style={{ animationDelay: "2.4s" }}><div className="glass glass-hover rounded-2xl p-5 h-full">
            <div className="flex items-center justify-between"><span className="mono text-xs text-slate-400">nexara roadmap</span><span className="mono text-xs text-violet-300">71%</span></div>
            <div className="mt-3 text-sm text-slate-400">10 / 14 milestones shipped</div>
            <div className="h-1.5 rounded-full bg-white/5 mt-3 overflow-hidden"><div className="h-full rounded-full" style={{ width:"71%", background:"linear-gradient(90deg,#7c3aed,#8b5cf6)", transition:"width 2s cubic-bezier(.16,1,.3,1)" }} /></div>
            <div className="mt-3 flex gap-1.5 flex-wrap">{["AI Agents","RAG","n8n","Vapi","SaaS"].map(t=>(<span key={t} className="px-2 py-0.5 rounded text-[10px] mono" style={{background:"rgba(139,92,246,0.1)",color:"#c4b5fd"}}>{t}</span>))}</div>
          </div></div></Reveal>
        </div>
      </section>
