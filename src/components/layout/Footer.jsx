...
export const Footer = memo(function Footer({ setPage }) {
  ...
  return (
    <footer className="relative z-10 mt-6 overflow-hidden">
      {/* top glow line */}
      <div className="h-px w-full" style={{ background: "linear-gradient(90deg,transparent,rgba(139,92,246,0.6) 30%,rgba(139,92,246,0.6) 70%,transparent)" }} />

      {/* ambient glow blobs */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute bottom-0 left-1/4 w-96 h-40 rounded-full opacity-20" style={{ background: "radial-gradient(circle,#7c3aed,transparent 70%)", filter: "blur(20px)" }} />
      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-14 pb-8">
        ...
        <Reveal>
          <div className="rounded-2xl mb-14 px-8 py-8 flex flex-col sm:flex-row items-center justify-between gap-6"
            style={{ background: "linear-gradient(135deg,rgba(124,58,237,0.12),rgba(139,92,246,0.15))", border: "1px solid rgba(139,92,246,0.3)" }}>
            <div>
        ...
        <div className="grid md:grid-cols-3 gap-12 mb-12">
          ...
          <Reveal className="space-y-5">
            <div className="flex items-center gap-3">
              <picture>
                ...
                <img src="/logo-40.png" alt="Nexara" width="40" height="40" className="w-10 h-10 object-contain shrink-0" style={{ filter: "drop-shadow(0 0 8px rgba(139,92,246,0.6))" }} />
              </picture>
              <div>
                <span className="mono text-base text-white font-medium">rehan.nazir<span className="text-violet-400">()</span></span>
            ...
            <p className="text-sm text-slate-400 leading-relaxed max-w-xs">
              AI Engineer &amp; Automation Specialist. Building intelligent, self-running systems. <span className="text-violet-300">Design. Build. Deploy. AI.</span>
            </p>
            <div className="mono text-xs text-slate-400 flex items-center gap-2">
              <span className="text-violet-400">//</span> Lahore, Pakistan · 2026
            </div>
          </Reveal>
          ...
          <Reveal delay={0.08}>
            <div className="mono text-xs uppercase tracking-widest text-slate-400 mb-5 flex items-center gap-2">
              <span className="text-violet-400">#</span> Navigate
            </div>
            <div className="flex flex-col gap-1">
              {navLinks.map(({ id, label, num }) => (
                <button
                  ...
                  data-cursor>
                  <span className="mono text-[11px] text-violet-400/60 group-hover:text-violet-400 transition-colors w-5">{num}</span>
                  <span className="text-sm text-slate-400 group-hover:text-white transition-colors">{label}</span>
                  <ChevronRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-violet-400 ml-auto opacity-0 group-hover:opacity-100 transition-all -translate-x-1 group-hover:translate-x-0" />
                </button>
              ))}
            </div>
          </Reveal>
          ...
          <Reveal delay={0.16}>
            <div className="mono text-xs uppercase tracking-widest text-slate-400 mb-5 flex items-center gap-2">
              <span className="text-violet-400">#</span> Connect
            </div>
            <div className="grid grid-cols-2 gap-3">
              {socials.map(({ Icon, label, href }) => (
                <a
                  ...
                  rel="noreferrer"
                  className="group flex items-center gap-2.5 px-3 py-2.5 rounded-xl glass transition-all duration-300 hover:border-violet-500/50"
                  ...
                  <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 group-hover:scale-110"
                    style={{ background: "linear-gradient(135deg,rgba(124,58,237,0.25),rgba(139,92,246,0.25))" }}>
