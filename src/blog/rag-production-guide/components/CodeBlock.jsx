...
/* Same VS-Code-dark shell and hand-rolled Python tokenizer as the Python-automation article's
   CodeBlock, plus the Code Presentation System additions from the approved design system:
   an execution badge (RUNNABLE vs ILLUSTRATIVE, so a reader never has to guess before pasting)
   and an optional version badge for samples with real API-surface version sensitivity. */
const KW = new Set([
...
const BT = new Set([
  ...
  "format","next","iter","abs","round","repr","id","hex","sqrt",
]);
...
const EXEC_STYLE = {
  runnable: { label: "RUNNABLE", bg: "rgba(16,185,129,0.14)", fg: "#34d399", border: "rgba(16,185,129,0.25)" },
  illustrative: { label: "ILLUSTRATIVE", bg: "rgba(245,158,11,0.14)", fg: "#fbbf24", border: "rgba(245,158,11,0.25)" },
};

export const CodeBlock = memo(function CodeBlock({ code, filename, note, lang = "python", execution = "runnable", version }) {
  ...
  const ex = EXEC_STYLE[execution] || EXEC_STYLE.runnable;
  return (
    <div className="my-8 rounded-2xl overflow-hidden not-prose" style={{
      ...
      <div className="flex items-center justify-between px-4 py-2.5 flex-wrap gap-2" style={{
        ...
        <div className="flex items-center gap-2">
          <span className="mono text-[10px] px-2 py-0.5 rounded font-medium" style={{ background: ex.bg, color: ex.fg, border: `1px solid ${ex.border}` }}>{ex.label}</span>
          {version && <span className="mono text-[10px] px-2 py-0.5 rounded" style={{ background: "rgba(148,163,184,0.14)", color: "#94a3b8", border: "1px solid rgba(148,163,184,0.2)" }}>{version}</span>}
